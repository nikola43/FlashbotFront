...
export class Web3Service {
  ...
  async getEstimatedTokensForBNB(tokenAddress) {
    ...
    if (pair !== '0x0000000000000000000000000000000000000000') {
      ...
      res = await lpTokenContract.methods.getReserves().call()
    } else {
      res = '0'
    }
  ...
  async generateToken(paymentToken, tokenName, tokenSymbol, tokenSupply, networkId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const gaiaContract = new window.web3.eth.Contract(
        ...
        const createPrice = await gaiaContract.methods.getRequiredEthAmount().call();

        const freeCreationNumber = await gaiaContract.methods.freeCreationNumber().call();

        const tokensCounter = await gaiaContract.methods.tokensCounter().call();

        const sendedValue = tokensCounter >= freeCreationNumber
          ? createPrice
          : 0;

        const create = await gaiaContract.methods
          .createNewTokenWithNative(
        ...
        console.log(b.logs)
        const contractAddress = b.logs[4].address;
  ...
  // tslint:disable-next-line:typedef
  async getTokenPair(tokenAddress) {
    const tokenContract = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    const getPairResult = await tokenContract.methods.lpPair().call();
  ...
  async lockLiquidity(tokenAddress: string, time: number, tokenAmount: number) {
    ...
    return await lockLiquidityContract.methods
      .lockTokens(
        await this.getTokenPair(tokenAddress),
        this.currentAccountSubject.value,
  ...
  async getLPTokensBalance(tokenAddress: string) {
    const pairAddress = await this.getTokenPair( tokenAddress);
    let res;
    if (pairAddress !== '0x0000000000000000000000000000000000000000') {
      const token = new window.web3.eth.Contract(LPTokenAbi, pairAddress);
      res = await token.methods.balanceOf(this.currentAccountSubject.value).call();
    } else {
      res = '0'
    }
    return res;
  ...
  // tslint:disable-next-line:typedef
  async getTokensSymbol(tokenAddress: string) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    return await token.methods.symbol().call();
  }

  // tslint:disable-next-line:typedef
  async getTokensSupply(tokenAddress: string) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    return Web3.utils.fromWei(await token.methods.totalSupply().call(), 'ether');
  }

  // tslint:disable-next-line:typedef
  async getTokensOwner(tokenAddress: string) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    return await token.methods.owner().call();
  }
  ...
  async isAllowed(address, spender) {
    const TokenBalance = await this.getTokensBalance(address);
    const isAddresAllowed =
      window.web3.utils.toWei(await this.getAddressAllowance(address, spender), 'ether') >=
      window.web3.utils.toWei(TokenBalance, 'ether');
  ...
  async isLPAllowed(address, spender) {
    ...
    const isAddresAllowed =
      window.web3.utils.toWei(await this.getLPAddressAllowance(address, spender), 'ether') > 0;
  ...
  async getLPAddressAllowance(address, spender) {
    const pairAddress = await this.getTokenPair(address);
  ...
  // tslint:disable-next-line:typedef
  async setAutoLiquidityPercent(tokenAddress, newVal) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    const result = await token.methods.setAutoLiquidityPercent(newVal).send({ from: this.currentAccountSubject.value });
    return result;
  }

  // tslint:disable-next-line:typedef
  async setW1AddressPercent(tokenAddress, newVal) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    const result = await token.methods.setW1AddressPercent(newVal).send({ from: this.currentAccountSubject.value });
    return result;
  }

  // tslint:disable-next-line:typedef
  async setW1Address(tokenAddress, newVal) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    const result = await token.methods.setW1Address(newVal).send({ from: this.currentAccountSubject.value });
    return result;
  }

  // tslint:disable-next-line:typedef
  async setBuyBackPercent(tokenAddress, newVal) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    const result = await token.methods.setBuyBackPercent(newVal).send({ from: this.currentAccountSubject.value });
    return result;
  }
  ...
  async transferOwner(tokenAddress, newOwner) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    const result = await token.methods.transferOwnership(newOwner).send({ from: this.currentAccountSubject.value });
    return result;
  }

  // tslint:disable-next-line:typedef
  async getTokenValues(tokenAddress) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);

    const swap = await token.methods.swapThreshold().call();
    const divisor = await token.methods.masterTaxDivisor().call();
    const maxWallet = await token.methods.maxWalletAmount().call();
    const addressP = await token.methods.w1AddressPercent().call();
    const autoLp = await token.methods.autoLiquidityPercent().call();
    const buyLmt = await token.methods.maxBuyLimit().call();
    const initDelayTime = await token.methods.initialDelayTime().call();
    const totalDelayTime = await token.methods.totalDelayTime().call();
    const priceLmt = await token.methods.maxGasPriceLimit().call();
    const delayBtw = await token.methods.timeDelayBetweenTx().call();
    const swapToken = await token.methods.swapTokenAddress().call();
    const lp = await token.methods.lpPair().call();
    const dead = await token.methods.DEAD().call();
    const tradingE = await token.methods.tradingEnabled().call();
    const gasLimit = await token.methods.gasLimitActive().call();
    const delay = await token.methods.transferDelayEnabled().call();
    const goldKing = await token.methods.midasGoldKing().call();
    const feeR = await token.methods._feesRates().call();
  ...
  // tslint:disable-next-line:typedef
  async renounceOwnership(tokenAddress) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    const result = await token.methods.renounceOwnership().send({ from: this.currentAccountSubject.value });
    return result;
  }
  ...
  // tslint:disable-next-line:typedef
  async setBuyBackThreshold(tokenAddress, newVal) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    const result = await token.methods.setBuyBackThreshold(newVal).send({ from: this.currentAccountSubject.value });
    return result;
  }
  ...
  // tslint:disable-next-line:typedef
  async excludeFromFee(tokenAddress, address, newVal) {
    const token = new window.web3.eth.Contract(TokenAbi, tokenAddress);
    console.log(newVal)
    const result = await token.methods.excludeFromFee(address, newVal).send({ from: this.currentAccountSubject.value });
    return result;
  }
  ...
  async getUserCreatedTokens(): Promise<string[]> {
    const gaiaTokenGenerator = new window.web3.eth.Contract(
      this.dataNetwork.GENERATOR_ABI,
      this.network[this.currentNetworkIdSubject.value].tokenCreatorContractAddress
    );
    return await gaiaTokenGenerator.methods.getUserCreatedTokens(this.currentAccountSubject.value).call();
  }
};

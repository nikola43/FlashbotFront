...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(
        ...
        masterTaxDivisor = masterTaxDivisor;
    ...
    function updateMaxBuyLimit(uint256 percent) public onlyOwner {
        maxBuyLimit = (totalSupply() * percent) / masterTaxDivisor;
    }
    ...
    function setW1AddressPercent(uint16 newPercent) external onlyOwner {
        require(
            newPercent + autoLiquidityPercent + buyBackPercent <=
                masterTaxDivisor,
    ...
    function setBuyBackPercent(uint16 newPercent) external onlyOwner {
        require(
            newPercent + autoLiquidityPercent + w1AddressPercent <=
                masterTaxDivisor,
            "Percent cant be higher than 100%"
        );
    ...
    function setAutoLiquidityPercent(uint16 newPercent) external onlyOwner {
        require(
            newPercent + buyBackPercent + w1AddressPercent <= masterTaxDivisor,
            "Percent cant be higher than 100%"
        );
    ...
    function _transfer(
        ...
        if (
            ...
            if (
                ...
                // in the first one hour, a maximum of XX BUSD purchase is adjustable (masterTaxDivisor BUSD is the default value)
                if (maxBuyLimit > 0) {
        ...
        if (contractMustSwap(from, to)) {
            ...
            if (buyBackPercent > 0) {
                midasGoldRouter.swapTokensForNativeToken(
                    address(this),
                    address(this),
                    (numTokensToSwap * buyBackPercent) / masterTaxDivisor
                );
            }
            ...
            if (buyBackPercent > 0) {
                ...
                if (ethBalance > buyBackThreshold) {
                    // buy back
                    midasGoldRouter.swapNativeForTokens(
                        address(this),
                        address(this),
                        ethBalance
                    );
    ...
    function setMaxWalletAmount(uint256 percent) public virtual onlyOwner {
        maxWalletAmount = (totalSupply() * percent) / masterTaxDivisor;
    }

    function setMaxTransactionAmount(uint256 percent) public virtual onlyOwner {
        maxTransactionAmount = (totalSupply() * percent) / masterTaxDivisor;
    }
}

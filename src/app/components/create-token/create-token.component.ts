...
export class CreateTokenComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('tokenName') tokenName!: ElementRef<HTMLElement>;
 ...
 */
      ...
      .pipe(
        ...
        map((d) => {
          if (d.errorMetamask) {
            ...
            console.log('error metamask', d);
            this.dialog.closeAll();
            this.tokenName.nativeElement.click()
            setTimeout(() => {
  ...
  // tslint:disable-next-line:typedef
  async onClickCopyToClipboard(address: string) {
    try {
  ...
  async getUserCreatedTokens() {
    this.myTokens = []
    await (await this.web3Service.getUserCreatedTokens()).forEach(async (r) => {
      this.myTokens.push({ name: await this.web3Service.getTokensName(r), symbol: await this.web3Service.getTokensSymbol(r), address: r});
      this.hasTokens$ = of(true)
      this.tokenName.nativeElement.click()
    })
